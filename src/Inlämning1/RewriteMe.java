...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return (int) questions.stream().filter(c -> c.getCategory() == category).collect(Collectors.toList()).stream().count();
    }
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(q -> q.getCategory() == category).map(Question::getQuestionString).collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream().flatMap(a -> a.answers.stream()).distinct().collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().flatMap(a -> a.answers.stream()).anyMatch(a -> a.equalsIgnoreCase(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().flatMap(a -> a.answers.stream()).filter(a1 -> a1.equalsIgnoreCase(answerCandidate)).count();
    }
    ...
    {
     return questions.stream().filter(q -> q.getCategory() == c).flatMap(a -> a.answers.stream()).reduce((a1, a2) -> {
